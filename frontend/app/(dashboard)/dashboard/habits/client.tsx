...
import { useTransition, useEffect } from "react";import { useRouter } from "next/navigation";
import { Button } from "@/components/ui/button";

export default function HabitClient({ scheduledHabits, executions }: { scheduledHabits: any[], executions: any[] }) {
  const [isPending, startTransition] = useTransition();  const router = useRouter();  useEffect(() => { const int = setInterval(() => router.refresh(), 5000); return () => clearInterval(int); }, [router]);
